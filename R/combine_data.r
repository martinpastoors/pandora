...
h1 <- loadRData(file.path(dropboxdir, "PFA data","pfa_20210413_haul_extraction_for_pandora.RData")) %>% 
  mutate(source="pfa")
l1 <- loadRData(file.path(dropboxdir, "PFA data","pfa_20210413_length_extraction_for_pandora.RData"))
...
h2 <- loadRData(file.path(dropboxdir, "SPFA data","spfa_ss_haul.RData")) %>% 
  rename(shootlon = londd, shootlat = latdd, date=dte, vid=vssl, catch=est_catch, species=sp) %>% 
  mutate(source="spfa", species=tolower(species))
l2 <- loadRData(file.path(dropboxdir, "SPFA data","spfa_ss_ltfreq.RData"))
